...
const requestPreMessageSentEvent = { jsonrpc: "2.0", id: "0mi75ya6hdzd", method: "app:executePreMessageSentExtend", params: [{ message: { id: "123", text: "test", room: {id: 'test'} } }] };
...
async function main() {
    ...
    const pipe = subprocess.stdout.pipe(new Transform({
        transform(chunk, encoding, callback) {
            console.log(new Date(), `RAW DENO: ${chunk.toString()}`);
            callback(null, chunk);
        }
    }));

    new Promise(async (resolve) => {
        for await (const message of decodeMultiStream(pipe)) {
            console.log(new Date(), 'DENO MESSAGE:', message);
            try {
                const rpc = jsonrpc.parseObject(message);

                if (rpc.type === 'request' && rpc.payload.id) {
                    console.log(new Date(), 'SENDING RESPONSE');
                    subprocess.stdin.write(encode(Object.assign({}, responseNull, { id: rpc.payload.id })));
                }
            } catch (error) {
    ...
    subprocess.stdin.write(encode(requestConstruct));

    await new Promise((resolve) => setTimeout(resolve, 1000));

    subprocess.stdin.write(encode(requestIinitialize));

    await new Promise((resolve) => setTimeout(resolve, 1000));

    subprocess.stdin.write(encode(requestGetStatus));

    await new Promise((resolve) => setTimeout(resolve, 1000));

    subprocess.stdin.write(encode(requestOnEnable));

    await new Promise((resolve) => setTimeout(resolve, 1000));

    subprocess.stdin.write(encode(requestSetStatus));
    ...
    subprocess.stdin.write(encode(requestSlashcommandExecutor));

    await new Promise((resolve) => setTimeout(resolve, 1000));

    subprocess.stdin.write(encode(requestPreMessageSentEvent));
